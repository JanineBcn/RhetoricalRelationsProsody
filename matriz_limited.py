...
folder = '/home/janine/Documents/phd/ted/ted_test/*.csv'
#change extension: ren *.txt *.csv
files = glob.glob(folder)
result = pd.DataFrame([])
result2 = pd.DataFrame([])
outdataFrame = pd.DataFrame([])
dir_name = '/home/janine/Documents/phd/ted/ted_test/out/'
number = 1
dfList = []
...
for infile in files:
	...
	for i in index_consecutives[0]:
		if depth[i] == nextDepth[i]:	
			if (depth[i] == nextDepth[i]) and (parent_rel[i] in ["elaboration_LeftToRight","background_LeftToRight","explanation_LeftToRight","contrast","background_RightToLeft","attribution_RightToLeft","manner-means_LeftToRight","condition_LeftToRight","attribution_LeftToRight","condition_RightToLeft","enablement_LeftToRight","contrast_RightToLeft"]):
				indices.append(i)
				indices2.append(i+1)
